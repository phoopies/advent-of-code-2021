...
class Cave:
    ...
    def _determine_type(id: str) -> CaveType:
        if id == CaveType.start.value: return CaveType.start
        if id == CaveType.end.value: return CaveType.end
        if id.islower(): return CaveType.small
        return CaveType.big 
...
class CaveSystem:
    ...
    def _make_paths(self, current_path: List[Cave], visited_small_twice: bool) -> Tuple[List[Cave], int]:
        if current_path[-1].is_end(): return current_path, 1 # made it to the end
        successes = 0
        for cave in current_path[-1].caves:
            if cave.is_start(): continue
            double_visit = visited_small_twice
            if cave.id in [cave.id for cave in current_path] and cave.is_small():
                if visited_small_twice: continue
                else: double_visit = True
