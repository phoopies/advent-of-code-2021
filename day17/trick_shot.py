...
class Probe:
    ...
    def handle_drag(self) -> None:
        direction = 0
        if self.x_vel > 0: direction = -Probe.drag
        elif self.x_vel < 0: direction = Probe.drag
...
class Launcher:
    ...
    def high_shot(self) -> int:
        ...
        Conservation of energy / momentum -> there exists some y != start_y so that y == start_y (== 0)
        Now we wan't the last step to be as big as possible -> y = - y_low, assuming y_low <= 0
        now realize that with max height shot we take the same amount of steps from y_start to y_top
        as from y_top to y (== 0)
        '''
    ...
    def fast_shot(self) -> int:
        '''
        Fastest shot we can do so that still hits the target
        '''
